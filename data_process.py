...
for num in range(0, 198):
    capture = cv2.VideoCapture("./src/videos_160/" + str(num) + ".mp4")
    ...
    while 1:
        ...
        frame_data = []     # 單一幀的資料
        frame_data.extend([num, frame_count])
        # /360 for normalization
        ...
        # 顯示
        cv2.imshow("video" + str(num), process)
        if not ref:
            break
...
# 以一部影片產生相關係數及熱度圖,幫助判斷部位分類
"""
corr_table = np.array(data)[:, 2:]
corr_col = ["right_arm_angle", "left_arm_angle", "right_leg_angle", "left_leg_angle", "waist_angle", "Neck_delta_x",
            "RShoulder_delta_x", "RElbow_delta_x", "RWrist_delta_x", "LShoulder_delta_x", "LElbow_delta_x",
            "LWrist_delta_x", "MidHip_delta_x", "RHip_delta_x", "RKnee_delta_x", "LHip_delta_x", "LKnee_delta_x",
            "Neck_delta_y", "RShoulder_delta_y", "RElbow_delta_y", "RWrist_delta_y", "LShoulder_delta_y",
            "LElbow_delta_y", "LWrist_delta_y", "MidHip_delta_y", "RHip_delta_y", "RKnee_delta_y", "LHip_delta_y",
            "LKnee_delta_y"]
corr_df = pd.DataFrame(corr_table, columns=corr_col)
print(corr_df)
plt.figure()
sns.heatmap(corr_df.corr(), cmap="rainbow", vmax=1, vmin=-1)
plt.tight_layout()
plt.savefig("corr.png")
plt.close("all")
corr_df.corr().to_csv("corr.csv")
"""
# 原data行名稱
...
arm_angle_df.to_csv("./src/csv/arm_angle.csv")
leg_angle_df = data_df[leg_angle_col]
leg_angle_df.to_csv("./src/csv/leg_angle.csv")
shoulder_df = data_df[shoulder_col]
shoulder_df.to_csv("./src/csv/shoulder.csv")
elbow_wrist_df = data_df[elbow_wrist_col]
elbow_wrist_df.to_csv("./src/csv/elbow_wrist.csv")
hip_knee_df = data_df[hip_knee_col]
hip_knee_df.to_csv("./src/csv/hip_knee.csv")
